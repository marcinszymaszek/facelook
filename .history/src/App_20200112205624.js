...
const App = () => {
  ...
  // const powers = data.members[0].powers;

  // const memberPowers = powers.map(power => {
  //   return <>
  ...
  const allUsers = members.members.map(user => {
    return <>
      <p className="basic-info-article">
        {/* <img src={user.avatar} alt='avatar' /><br /> */}
        User name: <strong>{user.secretIdentity}</strong>
  ...
  return (
    <>
      <div>
        <MainInfo usersAmount={members.members[0]} />
        <Users allUsers={members} />
      </div>
      {/* <div>{memberPowers}</div> */}
    </>
  )
}
